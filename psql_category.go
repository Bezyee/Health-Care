...
// NewCategoryRepositoryImpl tickt will create an object of PsqlCategoryRepository
func NewCategoryRepositoryImpl(Conn *sql.DB) *CategoryRepositoryImpl {
	return &CategoryRepositoryImpl{conn: Conn}
}

// Categories ticket returns all cateogories from the database
...
// Category ticket returns a category with a given id
...
// UpdateCategory ticket updates a given object with a new data
...
// DeleteCategory ticket removes a category from a database by its id
...
// StoreCategory ticket stores new category information to database
